...
import {
  ...
  summary,
  verification
} from "./entraAccessReviewControlPlaneService.js";
...
<html lang="en" data-skin="bert-dark">
  <head>
    ...
    <style>
      :root{
        /* bert-dark operator skin; --bert (accent) swapped green->cyan; state colors preserved */
      ...
      *{box-sizing:border-box}
      ...
      /* fallbacks match the bert-dark skin this page actually sets, so a
         404 on kg-theme-tokens.css degrades to the right palette, not the
         retired warm-ink one. */
      .kg-attest .dot{width:8px;height:8px;border-radius:50%;background:var(--kg-text-faint,#8C98A2)}
      .kg-attest[data-state="ok"] .dot{background:var(--kg-ok,#45A29E)}
      .kg-attest[data-state="stale"] .dot{background:var(--kg-warn,#E0A43A)}
      .kg-attest[data-state="fail"] .dot{background:var(--kg-danger,#F2495C)}
    </style>
  </head>
  <body>
    ...
    <div class="wrap">
      <div class="topbar">
        <div class="left">Kinetic Gain · Cloud Identity Control Plane</div>
        <div class="right">
          <div>tenant posture · synthetic review exports</div>
          <div>microsoft entra · access reviews · privileged roles</div>
        </div>
      </div>
      <div class="herorow">
        <section class="hero">
          <div class="chiprow">
            ...
            <span class="meta-chip">Azure / Microsoft 365 / Entra proof</span>
            <span class="meta-chip">Synthetic Microsoft Graph review packets</span>
          </div>
          <h1>Entra access reviews, privileged decisions, and remediation posture that stay governable.</h1>
          <p>This operator surface turns raw Microsoft Graph access-review exports into a buyer-readable control plane for Entra governance teams: overdue closeouts, self-reviews, auto-approvals, stale application gaps, and the remediation packet needed before the next audit window closes.</p>
          <div class="navrow">
        ...
        <aside class="side">
          <div class="bluf">
            ...
            <p><strong>Enterprise identity governance for Entra, Microsoft 365, and platform-security teams.</strong><br />Audit-safe review visibility, operator-friendly remediation, and privileged-role controls without tenant-secret exposure.</p>
          </div>
          <div class="corr">
            <div class="lbl">Proof Layer</div>
            <p><strong>Offline analyzer + CLI + dashboard surface.</strong><br />This repo is not only a static page. It includes a reusable analyzer that reads Microsoft Graph review exports and turns them into review posture and findings.</p>
          </div>
          <div class="corr">
            <div class="lbl">Why it matters</div>
            <p>Recruiters looking for <strong>Azure / Microsoft 365 / Entra / Intune</strong> need to see operator systems for real admin work: access reviews, privileged roles, remediation packets, and control-plane visibility.</p>
          </div>
        </aside>
      </div>
      ${body}
      <footer class="footer">
        ...
        <div>entra-access-review-control-plane · synthetic sample data only · <a href="https://kineticgain.com">kineticgain.com</a></div>
...
export function renderOverview() {
  ...
  return layout(
    ...
    `<section class="section">
        <div class="sh"><h2>Operator Snapshot</h2><div class="note">microsoft graph access-review lens</div></div>
        <div class="kpis">
          ...
          <div class="kpi red"><div class="v">${metrics.highFindings}</div><div class="lbl">high findings</div><div class="h">Privileged auto-approvals, self-reviews, or overdue closeouts.</div></div>
          ...
          <div class="kpi green"><div class="v">${metrics.privilegedAssignments}</div><div class="lbl">privileged assignments</div><div class="h">Privileged review rows surfaced for fast operator scanning.</div></div>
        </div>
      </section>
      <section class="section">
        ...
        <div class="stack">
          ...
          <div class="src"><div class="src-name">recruiter signal</div><div class="src-tit">Show Microsoft admin depth</div><p>This is a real Entra / Microsoft 365 governance surface, not a generic cloud keyword page.</p></div>
        </div>
      </section>`
  );
}
...
export function renderReviewLane() {
  return layout(
    "Entra Access Review Control Plane — Review Lane",
    "/review-lane",
    `<section class="section">
...
export function renderAccessRisks() {
  return layout(
    "Entra Access Review Control Plane — Access Risks",
    "/access-risks",
    `<section class="section">
...
export function renderRemediationPosture() {
  return layout(
    "Entra Access Review Control Plane — Remediation Posture",
    "/remediation-posture",
    `<section class="section">
...
export function renderVerification() {
  return layout(
    "Entra Access Review Control Plane — Verification",
    "/verification",
    `<section class="section">
        ...
        <div class="stack">
          ${verification()
            .map(
              (item, index) => `<div class="src"><div class="src-name">verification ${index + 1}</div><div class="src-tit">${item}</div><p>This surface is built to stay honest about offline exports, synthetic data, and Microsoft admin review posture.</p></div>`
            )
            .join("")}
        </div>
      </section>`
...
export function renderDocs() {
  return layout(
    "Entra Access Review Control Plane — Docs",
    "/docs",
    `<section class="section">
        ...
        <div class="stack">
          ...
          <div class="src"><div class="src-name">cli</div><div class="src-tit">Offline Graph export analysis</div><p><code>npx entra-access-review fixtures/reviews.json --format summary</code> renders the same underlying analyzer posture the dashboard exposes.</p></div>
        </div>
      </section>`
  );
}
