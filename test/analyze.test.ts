...
describe("analyze", () => {
  ...
  it("flags decision-overdue when instance closed > overdueAfterDays ago and decision is open", () => {
    ...
    // The Global Admin decision is still notReviewed → must be high severity.
    expect(od.some((f) => f.severity === "high" && (f.resource ?? "").includes("Global Administrator"))).toBe(true);
  });
  ...
  it("surfaces high-risk-principal info findings for every privileged-role decision", () => {
    ...
    // 3 privileged roles in the fixture (GA, SecAdmin, Helpdesk)
  ...
  it("counts openPrivilegedDecisions", () => {
    const r = analyze(fixture("reviews.json"), { now: NOW });
    expect(r.openPrivilegedDecisions).toBe(1); // only the notReviewed GA
  });
